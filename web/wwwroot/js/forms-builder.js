(() => {
    ...
    function renderBody(field) {
        ...
        if (isSection) {
            helpInput.rows = 3;
            helpInput.placeholder = 'Skriv den information respondenten skal se her, fx vejledning eller kontekst til formularen.';
        } else {
            helpInput.type = 'text';
        }
    ...
    function validate() {
        const errors = [];
        if (!els.titleInput.value.trim()) errors.push('Formularen skal have en titel.');
        state.fields.forEach((field, idx) => {
            ...
            if (field.fieldType !== 'SectionHeading' && !field.label.trim()) {
                errors.push(`Spørgsmål ${idx + 1} mangler en label.`);
            }
    ...
    function serializeToHiddenInputs() {
        ...
        state.fields.forEach((field, idx) => {
            ...
            const values = {
                [`Fields[${idx}].Id`]: field.id,
                [`Fields[${idx}].Label`]: field.label,
                [`Fields[${idx}].HelpText`]: field.helpText || '',
                [`Fields[${idx}].FieldType`]: field.fieldType,
                [`Fields[${idx}].IsRequired`]: field.isRequired ? 'true' : 'false',
                [`Fields[${idx}].Order`]: idx,
                [`Fields[${idx}].OptionsJson`]: options ? JSON.stringify(options) : ''
            };
